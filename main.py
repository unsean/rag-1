...
async def rag_ingest_pdf(ctx: inngest.Context):
    ...
    def _upsert(chunks_and_src: RAGChunkAndSrc) -> RAGUpsertResult:
        chunks = [c.strip() for c in chunks_and_src.chunks if c and c.strip()]
...
async def rag_query_pdf_ai(ctx: inngest.Context):
    def _search(question:str, top_k: int=10):
    ...
    res = await ctx.step.ai.infer(
        ...
        body={
            ...
            "temperature":0.2,
            "messages": [
                {"role": "system", "content": "Jawab pertanyaan dengan singkat dan langsung dalam Bahasa Indonesia. Jangan tunjukkan proses berpikir. Gunakan hanya konteks yang diberikan."},
